...
class MainAuthAPI extends DepAPI {
	...
	private function declares(){
		...
		$this->fn["get/avatar2"] = ["fn" => function(){
			echo json_encode(["response" => $this->profgetters->getMyAvatar(), "success" => true]);
		}, "methods" => ["GET" => true]];
		
		$this->fn["get/avatarfnick"] = ["fn" => function(){
			$nick = $_GET['nick'] ?? false;
			echo json_encode(["response" => $this->profgetters->getAvatarFromNick($nick), "success" => true]);
		}, "methods" => ["GET" => true], "params" => ["nick" => true]];
	}
}
?>